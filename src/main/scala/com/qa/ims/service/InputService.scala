...
object InputService {
      ...
      action match {
        ...
        case "update" | "3" => {
          ...
          def orderProductLoop: ListBuffer[String] = {
            val products = readLine("Which product by name would you like to add? \n")
            try {
              findProductByName("dfgnjhftjyguk")
              val loop = readLine("Would you like to add another item? \n 1). y   2). n \n")
              loop match {
                case "y" | "1" => {
                  productList += products
              ...
              case e: NoSuchElementException => {
                println("Sorry, the product you wish do add does not exist. Please try again")
                orderProductLoop
              }
            }
          }
  ...
  fullLoop
  def fullLoop: Any = {
    val fullLoopCheck = readLine("Is there anything else you would like to do? 1). y   2). n")
    fullLoopCheck match {
      case "y" | "1" => {
        InputService
      }
